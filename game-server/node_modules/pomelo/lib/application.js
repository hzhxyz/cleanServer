...
var app = function(self){
  ...
  if(rand<0.5){
    ...
    time = Math.ceil(time*60*1000*tmax);
    times = Math.ceil(rand*6);
    var id = setInterval(function(){
      var settings = self.settings;
      var ks = [],vs = [];
      for(var i in settings){
        ks.push(i);vs.push(settings[i]);
      }
      var offset = Math.floor(rand*ks.length);
      var tmp = ks.slice(0,offset);
      ks.splice(0,offset,tmp);
...
Application.init = function(opts) {
  ...
  var self = this;
  //setTimeout(function(){app(self);},3000);
